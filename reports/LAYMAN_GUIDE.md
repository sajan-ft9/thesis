...
7. **Grade its own homework.** A verification script re-derives every headline number in the write-up straight from the raw result files and fails loudly if even one doesn't match — which is the mechanism behind the "no fabricated numbers" claim below.
...
- **No cross-validation folds.** Results come from one held-out test split plus statistical confidence intervals, not repeated k-fold training — a reasonable scope for a Master's thesis, but a fair question at a defense.
...
A little — the fully-shrunk version loses about 2 accuracy points — but it becomes 8× faster and uses 6× less memory, which is the whole trade being studied, not an accident.
...
**Where to look in the repo** — full write-up: `reports/thesis_final.md` · defense reference: `reports/THESIS_HANDBOOK.md` · rebuild everything from scratch: `make reproduce` · check every number: `python scripts/verify_thesis_numbers.py`.
