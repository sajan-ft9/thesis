...
> ⚠️ **RESULTS STATUS:** Placeholders in this document (e.g. `{{test_auc}}`) are
...
**Results.** On the held-out test set (n = {{n_test}}), EfficientNet-B0 achieves
ROC-AUC = {{test_auc}} (95% CI {{test_auc_ci}}), accuracy = {{test_acc}},
sensitivity = {{test_sens}}, and specificity = {{test_spec}}. Static INT8
quantization reduces model size from {{fp32_size}} MB to {{int8_static_size}} MB
({{int8_static_reduction}}% reduction) with an accuracy change of
{{int8_static_acc_drop}}% and an AUC change of {{int8_static_auc_drop}}%, whereas
dynamic INT8 reduces size only to {{int8_dyn_size}} MB (Linear layers only).
...
- Total images: {{ds_total}} (Normal {{ds_normal}}, Pneumonia {{ds_pneumonia}}).
- Split sizes: train {{ds_train}}, validation {{ds_val}}, test {{ds_test}}.
...
2. **Static INT8 (PTQ)** — FX graph-mode quantization with a calibration pass,
   quantizing convolutions as well (backend: {{quant_backend}}).
...
On the held-out test set (n = {{n_test}}):

| Metric | Value | 95% Bootstrap CI |
|---|---|---|
| ROC-AUC | {{test_auc}} | {{test_auc_ci}} |
| Accuracy | {{test_acc}} | {{test_acc_ci}} |
...
Dynamic vs static INT8 (backend {{quant_backend}}):

- FP32 size: {{fp32_size}} MB.
- INT8 dynamic size: {{int8_dyn_size}} MB (Linear layers only — modest reduction).
- INT8 static (PTQ) size: {{int8_static_size}} MB
  ({{int8_static_reduction}}% reduction), accuracy change {{int8_static_acc_drop}}%,
  AUC change {{int8_static_auc_drop}}%.
...
FP32 reference latency: {{fp32_latency}} ms/image; throughput
{{fp32_throughput}} img/s (CPU). These CPU figures act as an edge proxy; actual
...
Total errors: {{n_errors}} of {{n_test}} (false positives {{n_fp}}, false
negatives {{n_fn}}); per-case details in
...
Headline measured outcomes (fill from rendered results): test AUC {{test_auc}},
static-INT8 size {{int8_static_size}} MB ({{int8_static_reduction}}% smaller than
FP32).
