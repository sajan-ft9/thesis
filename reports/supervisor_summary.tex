...
% (41/41 pass at time of writing). Full write-up: reports/thesis_final.md.
% =============================================================================
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  \item \textbf{Reproducibility.} \texttt{make reproduce} runs the full
        ...
        \texttt{results/metrics/*.json} verbatim (currently 41/41 pass).
\end{enumerate}
...
project's own verification script was re-run and confirmed to pass 41/41.
...
already observed for static-INT8 quantization. This was done only for the
primary model (not the two baselines), and fold assignment was image-level,
not patient-level (patient identifiers are not reliably recoverable from
the dataset's filenames) -- both are stated as remaining limitations below,
...
\begin{itemize}[leftmargin=1.4em]
  \item Single primary training dataset (pediatric); external validation is
        ...
        validation.
  \item 5-fold CV was run for the primary model only, not the two baseline
        architectures; fold assignment is image-level, not verified
        patient-level.
...
previous version of this summary has been directly addressed by the 5-fold
result above. The main open question for supervisory discussion is now
narrower: whether extending cross-validation to the two baseline
architectures and/or adding a second full external dataset is warranted
before the final submission, or whether the current single-model CV result
and single exploratory external probe are sufficient scope for a Master's
thesis.
